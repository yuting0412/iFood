...
def putSearch(table, count):
    ...
    for s in search:
        ...
        table.insert(st)
    # 餐點
    meal = ["早餐", "午餐", "晚餐", "早午餐", "宵夜", "下午茶", "甜點", "啤酒", "葡萄酒", "燒烤", "冰品飲料", "拉麵", "牛肉麵", "小吃", "提供素食料理", "咖啡", "火鍋", "咖哩", "牛排", "披薩", "海鮮", "壽司", "健康料理", "有機料理"]
    for m in meal:
        st = "{'CName':'" + m + "','IDPath':'0/396/397/" + str(count) + "','NamePath':'首頁/多元搜尋/餐點/" + m + "','nLevel':'3'}"
        count += 1
        table.insert(st)
    # 菜系
    cuisine = ["中式料理", "義式料理", "泰式料理", "日式料理", "港式料理", "美式料理", "韓式料理", "台灣小吃,台菜", "法式料理", "越南料理"]
    for c in cuisine:
        st = "{'CName':'" + c + "','IDPath':'0/396/398/" + str(count) + "','NamePath':'首頁/多元搜尋/菜系/" + c + "','nLevel':'3'}"
        count += 1
        table.insert(st)
    # 種類
    resType = ["精緻高級", "約會餐廳", "合菜", "親子餐廳", "景觀餐廳", "寵物友善", "居酒屋", "餐酒館", "酒吧" ,"咖啡廳", "吃到飽"]
    for t in resType:
        st = "{'CName':'" + t + "','IDPath':'0/396/399/" + str(count) + "','NamePath':'首頁/多元搜尋/種類/" + t + "','nLevel':'3'}"
        count += 1
        table.insert(st)
    # 價位
    price = ['100','100-200','200-300','300-400','400-500','500-600','600-700','700-800','800-900','900-1000','1000 - 1500','1500'] 
    for p in price:
        st = "{'CName':'" + p + "','IDPath':'0/396/400/" + str(count) + "','NamePath':'首頁/多元搜尋/價位/" + p + "','nLevel':'3'}"
        count += 1
        table.insert(st)
    # 評分
    score = ['1','2','3','4','5'] 
    for s in score:
        st = "{'CName':'" + s + "','IDPath':'0/396/401/" + str(count) + "','NamePath':'首頁/多元搜尋/評分/" + s + "','nLevel':'3'}"
        count += 1
        table.insert(st)
    # 特殊需求
    special = ["內用", "外帶", "外送", "需要預訂", "路邊取貨", "兒童高腳椅", "適合兒童", "適合闔家光臨", "適合團體", "Wi-Fi", "只收現金", "簽帳金融卡", "信用卡", "NFC 行動支付", "無障礙停車場", "無障礙洗手間", "無障礙座位", "洗手間", "室外雅座", "餐桌服務"]
    for s in special:
        st = "{'CName':'" + s + "','IDPath':'0/396/402/" + str(count) + "','NamePath':'首頁/多元搜尋/特殊需求/" + s + "','nLevel':'3'}"
        count += 1
        table.insert(st)
